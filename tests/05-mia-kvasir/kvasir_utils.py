...
class KvasirDataset:
    def __init__(self):
        ...
        # self.lbl_paths = np.random.permutation(np.sort(os.listdir(self.lbl_dir)))
...
def segmentation_train(train_dataloader, val_dataloader, epochs=5, lr=2e-4, encoder='mobilenet_v2'):
...
def attacker_train(attack_model, shadow_model, train_dataloader, opt, criterion):
    ...
    for data, labels, targets in train_dataloader:
        ...
        cat = torch.concat((s_output, cat), dim=1)

        output = attack_model(cat)
...
def attacker_val(attack_model, shadow_model, val_dataloader):
    pred_labels = np.array([])
    true_labels = np.array([])
...
def attacker_test(attack_model, victim_model, test_dataloader):
    pred_labels = np.array([])
    true_labels = np.array([])
    ...
    for data, labels, targets in test_dataloader:
        ...
        cat = labels.view(data.shape[0],1,data.shape[2],data.shape[3])
        cat = torch.concat((v_output, cat), dim=1)
...
class MIAbase:
    ...
    def train_victim(self, epochs=5, lr=1e-5):
        self.victim = segmentation_train(self.data.victim_dataloader_train, self.data.victim_dataloader_val, epochs=epochs, lr=lr)

    def train_shadow(self, epochs=5, lr=1e-5):
        self.shadow = segmentation_train(self.data.shadow_dataloader_train, self.data.shadow_dataloader_val, epochs=epochs, lr=lr)
    ...
    def train_attack(self, epochs=5, lr=1e-3):
        ...
        for epoch in range(epochs):
            print('***** EPOCH', epoch, '*****')
            self.attacker = attacker_train(self.attacker, self.shadow, self.data.attack_dataloader_train, opt, criterion)
            attacker_val(self.attacker, self.shadow, self.data.attack_dataloader_val)
            attacker_test(self.attacker, self.victim, self.data.attack_dataloader_test)