...
class LiverDataset:
    def __init__(self):
        ...
        # the function attack_data_merge() doesn't use the whole dataset -- not fixed because it prolongs training times 
        # and therefore overfitting; ideal fix: make the split already in data_split()
        print()
        print('*******************************************')
        print('*** DATA SPLIT DOESN\'T WORK AS INTENDED ***')
        print('*******************************************')
        print()

        print('Victim training images: {}'.format(len(self.victim_train_paths['imgs'])))
        print('Victim validation images: {}'.format(len(self.victim_val_paths['imgs'])))
        print('Shadow training images: {}'.format(len(self.shadow_train_paths['imgs'])))
        print('Shadow validation images: {}'.format(len(self.shadow_val_paths['imgs'])))

        print('Attack model training images: {}'.format(len(self.shadow_attack_paths['imgs'])))
        print('Attack model validation images: {}'.format(len(self.victim_attack_paths['imgs'])))

# -----------------------------------------------------------------------------------------------

class LiverLoader(data.Dataset):
    def __init__(self, data, attack=False, poison=False, poison_area=0):
    ...
    def __getitem__(self, index):
        ...
        # dataset poisoning: if the area of the segmented object is larger that certain amount, then it might get poisoned with 0.5 probability
        if (torch.sum(lbl) < self.poison_area) and (np.random.rand() < 0.5) and self.poison:
            lbl *= 0
