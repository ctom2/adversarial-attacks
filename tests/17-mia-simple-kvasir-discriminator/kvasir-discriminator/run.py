...
victim_train_ = KvasirLoader(data.victim_train_paths)
victim_train_dataloader = DataLoader(victim_train_, batch_size=int(VICTIM_BATCH_SIZE), shuffle=True)
victim_val_ = KvasirLoader(data.victim_val_paths)
...
shadow_train_ = KvasirLoader(data.shadow_train_paths)
shadow_train_dataloader = DataLoader(shadow_train_, batch_size=int(SHADOW_BATCH_SIZE), shuffle=True)
shadow_val_ = KvasirLoader(data.shadow_val_paths)
...
attack_train_ = KvasirLoader(data.shadow_attack_paths, attack=True)
attack_train_dataloader = DataLoader(attack_train_, batch_size=ATTACK_BATCH_SIZE, shuffle=True)
attack_val_ = KvasirLoader(data.victim_attack_paths, attack=True)
