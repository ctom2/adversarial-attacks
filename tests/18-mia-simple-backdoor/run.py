...
victim_train_ = LiverLoader(data.victim_train_paths, backdoor_train=True) # <----------------------------------- THE DATALOADER PRODUCES BACKDOORED SAMPLES
...
# backdoored dataloaders
victim_val_backdoored = LiverLoader(data.victim_val_paths, backdoor_test=True)
victim_val_backdoored_dataloader = DataLoader(victim_val_backdoored, batch_size=int(VICTIM_BATCH_SIZE))

print(' -- Staring victim model training --')
victim_model = train_segmentation_model_with_backdoor(
    victim_model, victim_train_dataloader, victim_val_dataloader, victim_val_backdoored_dataloader, lr=SEG_LR, epochs=VICTIM_TRAIN_EPOCHS)
