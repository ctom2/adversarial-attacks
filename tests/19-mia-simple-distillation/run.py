...
print('\n * Victim model == unprotected model, protected model == trained on reference dataset * \n')
...
victim_model, unprotected_val_loss = train_segmentation_model(victim_model, victim_train_dataloader, victim_val_dataloader, lr=SEG_LR, epochs=VICTIM_TRAIN_EPOCHS)

print(' -- Victim model trained --')
print('######################################################')

protected_model = smp.Unet(encoder_name=VICTIM_ENCODER, in_channels=1, classes=1).to(device)
print(' -- Protected model initialised --')
...
reference_idxs = get_reference_idxs(victim_model, pre_reference_dataloader, unprotected_val_loss)
...
shadow_model, _ = train_segmentation_model(shadow_model, shadow_train_dataloader, shadow_val_dataloader, lr=SEG_LR, epochs=SHADOW_TRAIN_EPOCHS)
